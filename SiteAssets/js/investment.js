...
const columns = [{
    ...
    "format": function(d) { return d.InvestmentNumber; },
    "type": "string",
    "display": false,
    "sort": false
}, {
    ...
    "format": function(d) { return d.InvestmentType; },
    "type": "string",
    "display": false,
    "sort": false
}, {
    ...
    "label": "% DME",
    "field": "PercentageDME",
    "format": function(d) { return perFormat(d.PercentageDME); },
    "type": "double",
    "display": true,
    "sort": false
}, {
    "label": "% SS",
    "field": "PercentageSS",
    "format": function(d) { return perFormat(d.PercentageSS); },
    "type": "double",
    "display": true,
    "sort": false
}, {
    "label": "DME",
    "field": "DME",
    "format": function(d) { return currFormat(d.DME); },
    "type": "double",
    "display": true,
    "sort": false
}, {
    "label": "SS",
    "field": "SS",
    "format": function(d) { return currFormat(d.SS); },
    "type": "double",
    "display": true,
    "sort": false
}, {
    "label": "Total",
    "field": "Total",
    "format": function(d) { return currFormat(d.Total); },
    "type": "double",
    "display": true,
    "sort": false
}, {
    ...
    "format": function(d) { return perFormat(d.PercentageChange); },
    "type": "double",
    "display": true,
    "sort": false
}];
...
function createViz(error, data) {
    ...
    const barChartHeight = (dashboardHeight - headerHeight - footerHeight - barChartHeaderHeight - barChartFooterHeight) * 1 / 4;
    ...
    let records = data.value.map(function(d) {
        d.Total = +d.Total * 1000;
        d.DME = +d.DME * 1000;
        d.SS = +d.SS * 1000;
        d.PercentageChange = +d.PercentageChange || 0;
    ...
    var
        ...
        investmentDim = ndx.dimension(function(d) {
    ...
    var
        ...
        amountByInvestment = investmentDim.group().reduce(
            function(p, v) {
                ++p.count;
        ...
        amountByDME = timeDim.group().reduce(
            function(p, v) {
                ++p.count;
                p.amount += Math.round(v.DME);
                p.total += Math.round(v.Total);
                p.percent = +(p.amount / p.total);
                return p;
            },
            function(p, v) {
                --p.count;
        ...
        amountBySS = timeDim.group().reduce(
            function(p, v) {
                ++p.count;
                p.amount += Math.round(v.SS);
                p.total += Math.round(v.Total);
                p.percent = +(p.amount / p.total);
                return p;
            },
    ...
    compositionBarChart
        ...
        .dimension(timeDim)
        .group(amountByDME, "DME")
        .valueAccessor(function(d) {
            return d.value.amount;
        })
        .stack(amountBySS, "Steady State", function(d) {
            return d.value.amount;
        })
        ...
        .legend(dc.legend().horizontal(true).x(barChartWidth / 2))
        .margins({
            top: margins.top,
            right: margins.right,
        ...
        .elasticY(true)
        .yAxisLabel("Total IT Spend per Year")
        .yAxis().ticks(Math.max(barChartHeight / 50, 2)).tickFormat(function(d) {
            return formatAbbreviation(d);
        });

    compositionBarChart.on('pretransition', function(chart) {
        setResponsiveSVG(chart);
    });

    compositionBarChart.addFilterHandler(function(filters, filter) {
        filters.length = 0; // empty the array
        filters.push(filter);
        return filters;
    });

    compositionBarChart.filter(2017);
    ...
    topTen
        ...
        .label(function(d) {
            if (topTen.hasFilter() && !topTen.hasFilter(d.key)) {
                return d.key + ' (0%)';
            }
            let label = d.key;
            if (all.value()) {
        ...
        .renderTitle(false)
        .elasticX(true)
        .xAxis().ticks(Math.max(barChartHeight / 50, 2)).tickFormat(function(d) {
            return formatAbbreviation(d);
        });
